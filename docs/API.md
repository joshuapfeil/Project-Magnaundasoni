...
| `mag_set_listener_head_pose` | `MagStatus mag_set_listener_head_pose(MagEngine engine, uint32_t listenerID, const float quaternion[4])` | Update a listener's head-tracked orientation using an `(x, y, z, w)` quaternion. |

### Spatialisation

| Function | Signature | Description |
|----------|-----------|-------------|
| `mag_set_spatial_config` | `MagStatus mag_set_spatial_config(MagEngine engine, const MagSpatialConfig* config)` | Configure built-in spatial routing (auto, binaural, passthrough, or surround presets). |
...
| `mag_set_speaker_layout` | `MagStatus mag_set_speaker_layout(MagEngine engine, const MagSpeakerLayout* layout)` | Override the current speaker layout with an explicit channel map. |
